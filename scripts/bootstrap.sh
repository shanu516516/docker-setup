...
get_btc_pubkey() {
  local wallet="$1"
  local result
  result=$(btc_rpc "${wallet}" "listdescriptors")
  # Get the second-to-last descriptor and extract the 66-char compressed pubkey
  echo "${result}" | jq -r '.result.descriptors[-2].desc' | grep -oP '\]\K[0-9a-fA-F]{66}'
}
